...
import { ChevronDown, Check, ChevronLeft, ChevronRight, MoreHorizontal } from "lucide-react";
...
export default function FutureSalesTable({ 
  ...
  const filteredSales = useMemo(() => {
    return futureSales.filter(sale => 
      filterStatus === "all" || 
      statuses.find(s => s.label === sale.status)?.id === filterStatus
    );
  }, [futureSales, filterStatus, statuses]);
  ...
  // If there are no future sales, don't render anything
  ...
  return (
    <Card className="mb-6">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div>
          <CardTitle className="text-lg font-medium">Ventas Futuras</CardTitle>
