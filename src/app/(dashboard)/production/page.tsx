...
interface Product {
  ...
  description: string;
  brand: string;
  business: "Mil Sabores" | "Torta Express"; // Nuevo campo para el negocio
  image?: string;
  recipe: RecipeItem[];
...
export default function ProductionPage() {
  ...
  const addProduction = () => {
    ...
    // Todas las producciones se crean como pendientes primero
    ...
    toast.success("Producción pendiente creada");
  ...
  const nextStep = () => {
    ...
    // Set default due date when moving to step 2
  ...
  const canStartProduction = (production: Production) => {
    if (production.status !== "pending") return false;
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {product.brand} - {product.business}
                        </p>
                      </div>
                      {selectedProducts.some((p) => p.id === product.id) ? (
                    ...
                    <CardContent className="p-3">
                      <div className="flex justify-between items-start">
                        <div className="flex items-start gap-3">
                          <div className="h-12 w-12 bg-gray-100 dark:bg-gray-700 rounded flex-shrink-0 flex items-center justify-center">
                            {product.image ? (
                              <img
                                src={product.image}
                                alt={product.name}
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 text-red-500 absolute top-1 right-1"
      ...
      case 3:
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="font-medium">Productos a producir</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {selectedProducts.map((product) => (
                  <Card key={product.id}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredProductions.map((production) => (
          <ProductionCard
            ...
            canStartProduction={canStartProduction(production)}
          />
        ))}
      </div>
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          {production.status === "pending" && (
            <Button 
              size="sm" 
              onClick={() => onStartProduction(production.id)}
      ...
      <div className="mt-3">
        ...
        <div className="grid grid-cols-1 gap-2 mt-2">
          {production.products.map((product) => (
            <div key={product.id} className="flex items-center gap-2 p-2 bg-white dark:bg-gray-800 rounded">
              <div className="h-8 w-8 bg-gray-100 dark:bg-gray-700 rounded flex-shrink-0 flex items-center justify-center">
                {product.image ? (
                  <img
                    src={product.image}
                    alt={product.name}
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {product.business} - {product.quantity} unidades
                </p>
              </div>
            </div>
          ))}
